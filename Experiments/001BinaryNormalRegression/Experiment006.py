...
def experiment6Main():
    ...
    for seed in SEEDS:
        X, y = PublicDS.get_credit_card_ds(seed)
        n_samples, n_features = X.shape
        n_classes = len(np.unique(y))
        print('MOHA:', n_samples, n_features, n_classes)
    ...
    olc_wa_sccm_acc = np.array(olc_wa_acc_list_per_seed).mean()

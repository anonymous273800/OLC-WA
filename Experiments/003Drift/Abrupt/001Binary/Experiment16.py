...
from Datasets.Drift.Abrupt.Binary import DS15, DS16
from Utils import Predictions, Measures, Printer

from Utils import Plotter
warnings.filterwarnings("ignore")

def experiment16Main():
    # Abrupt Drift dataset
    X, y, measures = DS16.get_DS16()
    ...
    C = 1
    start_time = time.perf_counter()
    pa_w, pa_b, pa_epoch_list, pa_cost_list, pa_acc_list = OnlinePassiveAggressive.online_passive_aggressive_binary(X_train, y_train, C, record_cost_on_every_epoch=100)
    ...
    learning_rate = .01
    start_time = time.perf_counter()
    w, b, widrow_hoff_epoch_list,widrow_hoff_cost_list, widrow_hoff_acc_list = WidrowHoff.widrow_hoff_classification(X_train, y_train, learning_rate, record_cost_on_every_epoch=100)
    ...
    perceptron_w, perceptron_b, perceptron_epoch_list, perceptron_cost_list, perceptron_acc_list  = \
                                                        Perceptron.perceptron(X_train, y_train, record_cost_on_every_epoch=100)
    end_time = time.perf_counter()
    ...
    online_logistic_regression_w, online_logistic_regression_b, online_logistic_regression_epoch_list, \
        online_logistic_regression_cost_list, online_logistic_regression_acc_list = Online_Logistic_Regression.online_logistic_regression(X_train, y_train, learning_rate, record_cost_on_every_epoch=100)
    end_time = time.perf_counter()
    ...
    naive_bayes_model, naive_bayes_epoch_list, naive_bayes_cost_list, naive_bayes_acc_list = \
        OnlineNaiveBayes.online_naive_bayes(X_train, y_train, record_cost_on_every_epoch=100)
    end_time = time.perf_counter()
    ...
    # 8. Hoeffding Tree
    start_time = time.perf_counter()
    hoeffding_tree_model, hoeffding_tree_epoch_list, hoeffding_tree_cost_list, hoeffding_tree_acc_list = HoeffdingTree.hoeffding_tree(X_train, y_train, record_cost_on_every_epoch=100)
    end_time = time.perf_counter()
    ...
    if (plotting_enabled):
        ...
        plot_results(x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, x_axis_pa, y_axis_pa,
                             ...
                             drift_location=5000, log_enabled=False,
...
def plot_results(x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, x_axis_pa, y_axis_pa,
    ...
    if drift_type == 'abrupt':
        ...
        plt.axvspan(drift_location, max(x_axis_olc_wa_sccm), color=Constants.color_yello, alpha=0.3, label='Concept Drift')

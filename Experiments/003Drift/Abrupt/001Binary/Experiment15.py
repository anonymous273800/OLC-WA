...
from Datasets.Drift.Abrupt.Binary import DS15
from Utils import Predictions, Measures, Printer

from Utils import Plotter
warnings.filterwarnings("ignore")

def experiment15Main():
    # Abrupt Drift dataset
    X, y, measures= DS15.get_DS15()
    ...
    #                                             OLCWA.olc_wa_lstm_sccm_classification(X_train, y_train,
    #                                                            .2,#Hyperparameters.olc_wa_w_inc,
    #                                                            Hyperparameters.olr_wa_base_model_size1,
    #                                                            30,#Hyperparameters.olr_wa_increment_size2(n_features,
    #                                                                #                             user_defined_val=10),
    #                                                            kpi=kpi,
    ...
    olr_wa_lstm_sccm_coeff, olr_wa_lstm_sccm_epoch_list, olr_wa_lstm_sccm_cost_list, olr_wa_lstm_sccm_acc_list = \
        OLCWA.olc_wa_lstm_sccm_classification(X_train, y_train,
                                              .2,  # Hyperparameters.olc_wa_w_inc,
                                              Hyperparameters.olr_wa_base_model_size1,
                                              25,  # Hyperparameters.olr_wa_increment_size2(n_features,
                                              #                             user_defined_val=10),
                                              kpi=kpi,
                                              multiplier=multiplier_acc,
                                              expr='expr15'
    ...
    C = 1
    start_time = time.perf_counter()
    pa_w, pa_b, pa_epoch_list, pa_cost_list, pa_acc_list = OnlinePassiveAggressive.online_passive_aggressive_binary(X_train, y_train, C, record_cost_on_every_epoch=10)
    ...
    learning_rate = 0.1
    start_time = time.perf_counter()
    w, b, widrow_hoff_epoch_list,widrow_hoff_cost_list, widrow_hoff_acc_list = WidrowHoff.widrow_hoff_classification(X_train, y_train, learning_rate, record_cost_on_every_epoch=10)
    ...
    perceptron_w, perceptron_b, perceptron_epoch_list, perceptron_cost_list, perceptron_acc_list  = \
                                                        Perceptron.perceptron(X_train, y_train, record_cost_on_every_epoch=10)
    end_time = time.perf_counter()
    ...
    online_logistic_regression_w, online_logistic_regression_b, online_logistic_regression_epoch_list, \
        online_logistic_regression_cost_list, online_logistic_regression_acc_list = Online_Logistic_Regression.online_logistic_regression(X_train, y_train, learning_rate, record_cost_on_every_epoch=10)
    end_time = time.perf_counter()
    ...
    naive_bayes_model, naive_bayes_epoch_list, naive_bayes_cost_list, naive_bayes_acc_list = \
        OnlineNaiveBayes.online_naive_bayes(X_train, y_train, record_cost_on_every_epoch=10)
    end_time = time.perf_counter()
    ...
    # 8. Hoeffding Tree
    start_time = time.perf_counter()
    hoeffding_tree_model, hoeffding_tree_epoch_list, hoeffding_tree_cost_list, hoeffding_tree_acc_list = HoeffdingTree.hoeffding_tree(X_train, y_train, record_cost_on_every_epoch=10)
    end_time = time.perf_counter()
    ...
    if (plotting_enabled):
        ...
        plot_results(x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, x_axis_pa, y_axis_pa,
                     ...
                     drift_location=500, log_enabled=False,
                     legend_loc='lower left', drift_type='abrupt', gradual_drift_locations=None,
...
def plot_results(x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, x_axis_pa, y_axis_pa,
    ...
    line1, = plt.plot(x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, linestyle='-', marker='o', markersize=3, linewidth=2, label=label_olc_wa, zorder=10)

    line2, = plt.plot(x_axis_pa, y_axis_pa, linestyle='-', marker='.', markersize=1, linewidth=0.9, label=label_pa,zorder=9)
    ...
    # # Line 1 - Clear and prominent
    # line1, = plt.plot(x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, linestyle='-', marker='o', markersize=4, linewidth=2,
    #                   color='#1f77b4', label=label_olc_wa, zorder=10)
    #
    # # Line 2 - Slightly less clear, distinct color
    # line2, = plt.plot(x_axis_pa, y_axis_pa, linestyle='-', marker='.', markersize=2, linewidth=1.1,
    #                   color='g', label=label_pa, zorder=5)
    #
    # # Line 3 - Slightly less clear, distinct color
    # line3, = plt.plot(x_axis_widrow_hoff, y_axis_widrow_hoff, linestyle='-', marker='.', markersize=2, linewidth=1.1,
    #                   color='r', label=label_lms, zorder=5)
    #
    # # Line 4 - Slightly less clear, distinct color
    # line4, = plt.plot(x_axis_perceptron, y_axis_perceptron, linestyle='-', marker='.', markersize=2, linewidth=1.1,
    #                   color='c', label=label_pla, zorder=5)
    #
    # # Line 5 - Slightly less clear, distinct color
    # line5, = plt.plot(x_axis_online_logistic_regression, y_axis_online_logistic_regression, linestyle='-', marker='.',
    #                   markersize=2, linewidth=1.1, color='m', label=label_olr, zorder=5)
    #
    # # Line 6 - Slightly less clear, distinct color
    # line6, = plt.plot(x_axis_naive_bayes, y_axis_naive_bayes, linestyle='-', marker='.', markersize=2, linewidth=1.1,
    #                   color='y', label=label_nb, zorder=5)
    #
    # # Line 7 - Slightly less clear, distinct color
    # line7, = plt.plot(x_axis_hoeffding_tree, y_axis_hoeffding_tree, linestyle='-', marker='.', markersize=2,
    #                   linewidth=1.1,
    #                   color='orange', label=label_ht, zorder=5)
    ...
    if drift_type == 'abrupt':
        plt.legend(fontsize='x-small', loc=legend_loc, fancybox=True, shadow=True, borderpad=1, labelspacing=0,

...
from Datasets.Drift.Abrupt.Multiclass import DS18
import time
...
def experiment17Main():
    X, y, measures = DS18.get_DS18()
    n_samples, n_features = X.shape
    print(n_samples, n_features)
    n_classes = len(np.unique(y))

    train_percent = int(85 * n_samples / 100)
    ...
    olc_wa_lstm_sccm_ovr_coeff_classifiers, olc_wa_lstm_sccm_ovr_epoch_list, olc_wa_lstm_sccm_ovr_cost_list, \
        ...
        OLCWA_MC_ONE_VS_REST.olc_wa_lstm_sccm_ovr_classification(X_train, y_train,
                                                                 .1, #Hyperparameters.olc_wa_w_inc,
                                                                 Hyperparameters.olr_wa_base_model_size1,
                                                                 Hyperparameters.olr_wa_increment_size2(n_features,
                                                                                                  user_defined_val=10),
                                                                 kpi=kpi, multiplier=multiplier_acc, expr='expr18')
    ...
    pa_w, pa_b, pa_epoch_list, pa_cost_list, pa_acc_list = OnlinePassiveAggressive.online_passive_aggressive_multiclass_sklearn2(
        X_train, y_train, C, record_cost_on_every_epoch=100)
    ...
    Printer.print_list_tabulate(lms_acc_list)
    ...
    perceptron_classifiers, perceptron_epoch_lists, perceptron_cost_lists, perceptron_acc_list = \
        Perceptron.perceptron_mc_ovr(X_train, y_train, record_cost_at_each_epochs=100)
    ...
    olr_classifiers, olr_epoch_lists, olr_cost_lists,  olr_acc_lists = Online_Logistic_Regression.online_logistic_regression_ovr_final(
        X_train, y_train, learning_rate, record_cost_on_every_epoch=100)
    end_time = time.perf_counter()
    ...
    Printer.print_list_tabulate(nb_acc_list)
    ...
    ht_classifier, ht_epoch_lists, ht_cost_lists, ht_acc_list = \
         HoeffdingTree.hoeffding_tree(X_train, y_train, record_cost_on_every_epoch=100)
    # compute acc for test data:
    ...
    Printer.print_list_tabulate(ht_acc_list)
    ...
    if (plotting_enabled):
        ...
        plot_results( x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, x_axis_pa, y_axis_pa,
                     ...
                     drift_location=7500, log_enabled=False,

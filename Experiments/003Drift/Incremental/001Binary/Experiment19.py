...
from Datasets.Drift.Incremental.Binary import DS19
...
def experiment19Main():
    ...
    train_percent = int(93 * n_samples / 100)
    ...
    olr_wa_lstm_sccm_coeff, olr_wa_lstm_sccm_epoch_list, olr_wa_lstm_sccm_cost_list, olr_wa_lstm_sccm_acc_list = \
                                                OLCWAFinal.olc_wa_lstm_sccm_classification(X_train, y_train,
                                                               .4,#Hyperparameters.olc_wa_w_inc,
                                                               #Hyperparameters.olr_wa_base_model_size1,
                                                               50, #Hyperparameters.olr_wa_increment_size(n_features,
                                                                   #                             user_defined_val=10),
                                                               kpi=kpi,
    ...
    C = .1
    start_time = time.perf_counter()
    pa_w, pa_b, pa_epoch_list, pa_cost_list, pa_acc_list = OnlinePassiveAggressive.online_passive_aggressive_binary(X_train, y_train, C, record_cost_on_every_epoch=50)
    ...
    learning_rate = 0.1
    start_time = time.perf_counter()
    w, b, widrow_hoff_epoch_list,widrow_hoff_cost_list, widrow_hoff_acc_list = WidrowHoff.widrow_hoff_classification(X_train, y_train, learning_rate, record_cost_on_every_epoch=50)
    ...
    perceptron_w, perceptron_b, perceptron_epoch_list, perceptron_cost_list, perceptron_acc_list  = \
                                                        Perceptron.perceptron(X_train, y_train, record_cost_on_every_epoch=50)
    end_time = time.perf_counter()
    ...
    online_logistic_regression_w, online_logistic_regression_b, online_logistic_regression_epoch_list, \
        online_logistic_regression_cost_list, online_logistic_regression_acc_list = Online_Logistic_Regression.online_logistic_regression(X_train, y_train, learning_rate, record_cost_on_every_epoch=50)
    end_time = time.perf_counter()
    ...
    naive_bayes_model, naive_bayes_epoch_list, naive_bayes_cost_list, naive_bayes_acc_list = \
        OnlineNaiveBayes.online_naive_bayes(X_train, y_train, record_cost_on_every_epoch=50)
    end_time = time.perf_counter()
    ...
    # 8. Hoeffding Tree
    start_time = time.perf_counter()
    hoeffding_tree_model, hoeffding_tree_epoch_list, hoeffding_tree_cost_list, hoeffding_tree_acc_list = HoeffdingTree.hoeffding_tree(X_train, y_train, record_cost_on_every_epoch=50)
    end_time = time.perf_counter()
    ...
    if (plotting_enabled):
        ...
        plot_results(x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, x_axis_pa, y_axis_pa,
                     ...
                     incremental_drift_locations=[600, 800, 1000, 1200, 1400, 1600, 1800, 2000, 2200]                     )
...
def plot_results(x_axis_olc_wa_sccm, y_axis_olc_wa_sccm, x_axis_pa, y_axis_pa,
                 ...
                 legend_loc, drift_type, incremental_drift_locations):
    ...
    # # Adding legend
    # if drift_type == 'incremental':
    #     plt.legend(handles=[line1, line2, line3, line4, line5, line6, line7, concept_drift_line], fontsize='small', loc=legend_loc, fancybox=True,
    #                shadow=True,

...
def add_mini_batch_statistics_to_memory(Xj, yj, base_classifiers, memoryManager, recomputed, curr_acc):
    acc = PredictionsMultiClass.compute_acc_olr_wa_ovr(Xj, yj, base_classifiers)
    if recomputed:
    ...
    if (curr_acc is not None and acc < curr_acc):
        acc = curr_acc
        new_acc_updated = True
...
def olc_wa_lstm_sccm_ovr_classification(X, y, w_inc, base_model_size, increment_size, kpi, multiplier, expr):
    ...
    for iteration, mini_batch_uuid, Xj, yj in mini_batch_generator:
        ...
        curr_acc, new_acc_updated = add_mini_batch_statistics_to_memory(Xj, yj, base_classifiers, memoryManager, recomputed=False, curr_acc=None)
        ...
        if (len(memoryManager.mini_batch_data) >= 4):
            ...
            if (ST_drift_detected):
                ...
                curr_acc, new_acc_updated = add_mini_batch_statistics_to_memory(Xj, yj, avg_planes, memoryManager, recomputed=True, curr_acc=curr_acc)
                if new_acc_updated:

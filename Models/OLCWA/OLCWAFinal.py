...
def olc_wa_lstm_sccm_classification(X, y, w_inc, increment_size, kpi, multiplier, expr):
    ...
    for iteration, mini_batch_uuid, Xj, yj in mini_batch_generator:
        ...
        inc_model_predicted_y = Predictions.predict(Xj, inc_model_w, inc_model_b)
        inc_coeff = np.array(np.append(np.append(inc_model_w, -1), inc_model_b))
        cost = Measures.cross_entopy_cost(yj, inc_model_predicted_y)
        cost_list = np.append(cost_list, cost)

        inc_acc = Measures.accuracy(yj, inc_model_predicted_y)
        acc_list = np.append(acc_list, inc_acc)
        print("mine inc_acc",inc_acc)
        ...
        avg_model_predicted_y_test = Predictions.predict_(Xj, avg_plane)
        avg_plane_accuracy = Measures.accuracy(yj, avg_model_predicted_y_test)
        print("MOHA avg_model_accuracy", avg_plane_accuracy)

        avg_model_acc = 0
        if inc_acc > avg_plane_accuracy:
            avg_plane = inc_coeff
            avg_model_acc = inc_acc
        else:
            avg_plane = avg_plane
            avg_model_acc = avg_plane_accuracy
        ...
        if (len(memoryManager.mini_batch_data) >= 4):
            ...
            if (ST_drift_detected):
                ...
                print('acc_from_stt', acc_from_stt)
                if (acc_from_stt > avg_model_acc):
                    avg_model_acc = acc_from_stt
                    avg_plane = avg_plane_from_st_trn
